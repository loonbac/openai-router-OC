...
function startInlineRouter() {
    const app = new Hono();
    let pluginConf = { ...DEFAULT_CONFIG };
    // Local helpers (avoid duplicate top-level definitions)
    const localDecodeJWT = (token) => {
        try {
            const parts = token.split('.');
            if (parts.length !== 3)
                return null;
    ...
    const localNormalizeModel = (model) => {
        if (!model)
            return 'gpt-5.1';
        const modelId = model.includes('/') ? model.split('/').pop() : model;
        return modelId.replace(/-(?:fast|none|minimal|low|medium|high|xhigh)$/, '');
    };
    const localIsSparkModel = (model) => {
        return typeof model === 'string' && model.startsWith('gpt-5.3-codex-spark');
    };
    const localSupportsFastMode = (model) => {
        return model === 'gpt-5.5' || model === 'gpt-5.4';
    };
    function localTransformSSEEvent(codexEvent) {
    ...
    app.get('/health', (c) => {
        ...
        const now = Date.now();
        const eligible = Object.values(store.accounts).filter(acc => (!acc.rateLimitedUntil || acc.rateLimitedUntil < now) &&
    ...
    app.post('/v1/chat/completions', async (c) => {
        ...
        const effectiveConfig = { ...pluginConf, rotationStrategy: settings.settings.rotationStrategy };
        const maxAttempts = 5;
        ...
        while (attempt < maxAttempts) {
            ...
            if (reasoningMatch?.[1]) {
                ...
                if (!localIsSparkModel(normalizedModel))
                    ...
                    start(controller) {
                        fetch(CODEX_RESPONSES_URL, {
                            ...
                            if (!res.ok) {
                                ...
                                if (res.status === 429)
                                    markRateLimited(account.alias, Date.now() + 60000);
                                if (!streamEnded) {
...
const MultiAuthPlugin = async ({ client, $, serverUrl, project, directory }) => {
     ...
     */
    const notifyEvent = async (title, message) => {
        if (!notifyEnabled)
            return;
        try {
